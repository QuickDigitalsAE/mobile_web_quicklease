        ...
        swal({
            ...
            .then(willDelete => {
                if (willDelete) {
                    ...
                    // apiMethodDelete(`sidebarBanner/${id}`)
                }
...
const check = (module, action) => permission?.[module]?.includes(action);
    return (
        <div className='BlogsSidebarCard relative shadow-custom border border-[#D4DEF1] rounded-3xl bg-white'>
            {/* {check("SidebarBanners", "SidebarBanners Delete") &&<div className='closeButton cursor-pointer absolute bg-[#FFCEDA] w-[2rem] h-[2rem] p-[.6rem] top-[.7rem] right-[.7rem] grid place-items-center rounded-[.7rem] z-10' onClick={handleDelete}>
            ...
            <Link className="BlogsSidebarCardMain p-4 grid grid-cols-[4fr,6fr] gap-4 max-lg:grid-cols-1" to={`/blogs/sidebar/edit/${id}`}>
                <div className="BlogsSidebarCard__left max-lg:mb-4">
                    <img src={banner_image} className='w-full h-[10rem] object-cover rounded-3xl' alt="1.png" />
                </div>
                <div className="BlogsSidebarCard__leftRight pr-8">
                    <div className="h2 text-[1.25rem] capitalize font-MluvkaLight leading-[1.2] mb-2">{title}</div>
                    <div className='text-[#393946] text-[.8rem] leading-[1.5] mb-4'>{redirect_url}</div>
                </div>
            </Link>
        </div>
    )
...
export default BlogsSidebarCard
