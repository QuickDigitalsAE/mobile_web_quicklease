    ...
    const debounce = (func, delay) => {
      ...
      const executeApiCall = (e) => {
        ...
        if ((e.target.value).trim()) {
            apiMethod2(`partners/search_partner_list/${mainLanguage}/6?page=${currentPage}`, formdata);
        } else {
            apiMethodGet(`partners/list/${mainLanguage}/6?page=${currentPage}`, formdata);
        }
      };
    ...
    return (
        <div className='NewsPage pr-10 max-lg:pr-6'>
            <div className="TeamPageTop flex justify-between items-center">
                <h6 className='text-[1rem] mb-2 bookingSectionh relative px-3 font-Mluvka capitalize'>Partners and updates</h6>
                <div className='flex gap-1'>
                    ...
                    {check("Partners", "Partner Add") &&<Link to={"/partners/create"} className='bg-[#d9dcf8] py-3 px-6 rounded-full flex items-center gap-2 cursor-pointer'>
                        <img src={plus} alt="plus" />
                        <span className='font-MluvkaBold text-secondary capitalize'>Add Partners</span>
                    </Link>}
                </div>
            </div>
            ...
            {resget.isLoading ? 
                        ...
                        Array.isArray(datas) && datas.map((item, index) => {
                            return (
                                <React.Fragment key={index}>
                                    <PartnersCard permission={permission} data={item} alldata={datas} deleted={setDatas}  />
                                </React.Fragment>
                            )
                        })}
            </div>
