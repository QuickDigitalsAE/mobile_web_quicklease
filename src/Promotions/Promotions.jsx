...
const Promotions = ({permission}) => {
    ...
    const debounce = (func, delay) => {
      ...
      const executeApiCall = (e) => {
        ...
        if ((e.target.value).trim()) {
            apiMethod2(`promotions/search_promotions_list/${mainLanguage}/6?page=${currentPage}`, formdata);
        } else {
            apiMethodGet(`promotions/list/${mainLanguage}/6?page=${currentPage}`, formdata);
        }
      };
  ...
  return (
    ...
    <div className="TeamPageTop flex justify-between items-center">
                <h6 className='text-[1rem] mb-2 bookingSectionh relative px-3 font-Mluvka capitalize'>Promotions and updates</h6>
                <div className='flex gap-1'>
                    ...
                    {check("Promotions", "Promotion Add") && <Link to={"/promotions/create"} className='bg-[#d9dcf8] py-3 px-6 rounded-full flex items-center gap-2 cursor-pointer'>
                        <img src={plus} alt="plus" />
                        <span className='font-MluvkaBold text-secondary capitalize'>Add Promotions</span>
                    </Link>}
                </div>
            </div>
            ...
            {resget.isLoading ? 
                        ...
                        Array.isArray(datas) && datas.map((item, index) => {
                            return (
                                <React.Fragment key={index}>
                                    <PromotionsCard permission={permission} data={item} alldata={datas} deleted={setDatas}  />
                                </React.Fragment>
                            )
                        })}
            </div>
