...
const PromotionsCard = ({data,alldata,deleted,permission}) => {
        ...
        swal({
            ...
            .then(willDelete => {
                if (willDelete) {
                    ...
                    // apiMethodDelete(`promotions/delete/${id}`)
                }
    ...
    useEffect(() => {
      ...
      const maxLength = 200;
      const truncatedText = truncateText(promotion_short_paragraph, maxLength);
const check = (module, action) => permission?.[module]?.includes(action);
    return (
        <div className='BlogsCard relative shadow-custom border border-[#D4DEF1] rounded-3xl bg-white'>
            {/* {check("Promotions", "Promotion Delete") &&<div className='closeButton cursor-pointer absolute bg-[#FFCEDA] w-[2rem] h-[2rem] p-[.6rem] top-[.7rem] right-[.7rem] grid place-items-center rounded-[.7rem] z-10' onClick={handleDelete}>
            ...
            <Link className="BlogsCardMain p-4 grid grid-cols-[4fr,6fr] gap-4 max-lg:grid-cols-1" to={`/promotions/edit/${id}`}>
                <div className="BlogsCard__left max-lg:mb-4">
                    <img src={promotion_image} className='w-full h-[10rem] object-cover rounded-3xl' alt="1.png" />
                </div>
                <div className="BlogsCard__leftRight pr-8">
                    <div className="h2 text-[1.25rem] capitalize font-MluvkaLight leading-[1.2] mb-2">{promotion_title}</div>
