...
const EditProductsProperties = ({ permission }) => {
  ...
  useEffect(() => {
    apiMethodGet(`properties/edit/${id}/${mainLanguage}`)
  }, [mainLanguage]);
  ...
  const handleSubmit = async (values) => {
    if (imageLoader) {
      ...
      if (checkerRequried.length > 0) {
        ...
        apiMethod(`properties/update/${id}/${mainLanguage}`, formdata)
      }
  ...
  if (!datas || resget?.isLoading) return <SkeletonCreateEdit heading={"Products Properties"} />;

  let initialValues = {
    type: resget?.data?.data?.type,
    property_title: resget?.data?.data?.property_title,
    property_field_type: resget?.data?.data?.property_field_type,
    property_status: resget?.data?.data?.property_status ?? 0,
  };
  ...
  return (
    <div className='newscreate pr-10 max-lg:pr-6'>
      <Link to={"/products/properties"} className="back flex items-center mb-6 gap-2">
        ...
        <span className='text-[1.4rem] font-MluvkaBold'>Edit Products Properties</span>
      </Link>
        ...
        <Form name="myForm">
          ...
          <div className='bg-[#EFF4FD] p-6 rounded-3xl mb-3'>
            ...
            <div className='flex justify-between mt-5'>
              ...
              <Link className='bg-[#d9dcf8] py-3 px-6 rounded-full flex items-center gap-2 cursor-pointer' onClick={() => handleSectionAdd("property_values")} >
          ...
          {check("ProductProperties", "ProductProperties Edit") && <SubmitButton
            props={{
...
export default EditProductsProperties
