...
const ProductsProperties = ({permission}) => {
    ...
    const onChange = (current, pageSize) => {
        ...
        if ((searchValue).trim()) {
            apiMethod2(`properties/search_properties_list/${mainLanguage}/6?page=${current}`, formdata);
        } else {
            apiMethodGet(`properties/list/${mainLanguage}/6?page=${current}`, formdata);
        }
    };
    ...
    useEffect(() => {
        if (mainLanguage) {
            setCurrentPage(1)
            apiMethodGet(`properties/list/${mainLanguage}/6?page=1`);
        }
    }, [mainLanguage]);
    ...
    const debounce = (func, delay) => {
      ...
      const executeApiCall = (e) => {
        ...
        if ((e.target.value).trim()) {
            apiMethod2(`properties/search_properties_list/${mainLanguage}/6?page=${currentPage}`, formdata);
        } else {
            apiMethodGet(`properties/list/${mainLanguage}/6?page=${currentPage}`, formdata);
        }
      };
  ...
  return (
    ...
    <div className="TeamPageTop flex justify-between items-center">
                <h6 className='text-[1rem] mb-2 bookingSectionh relative px-3 font-Mluvka capitalize'>Properties and updates</h6>
            ...
            {resget.isLoading ? 
                        ...
                        Array.isArray(datas) && datas.map((item, index) => {
                            return (
                                <React.Fragment key={index}>
                                <ProductsPropertiesCard permission={permission} data={item} alldata={datas} deleted={setDatas}  />
                                </React.Fragment>
                            )
                        })}
            </div>
