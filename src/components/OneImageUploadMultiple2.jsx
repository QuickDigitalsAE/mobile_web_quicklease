...
const OneImageUploadMultiple2 = ({Update,changeImage,section,indexValue,sec_image,sec_value,folder_name,page_type,MainImage}) => {
        ...
        if (e.dataTransfer.files[0] && e.dataTransfer.files[0]?.type?.includes("image")) {
            ...
            Update(prevService => ({
                ...
                [section]: prevService[section].map((item,index2) =>
                  index2 === indexValue ? { ...item, [sec_image]: path } : item
              )
            }));
    ...
    if (res2.data) {
      ...
      if (status === "false") {
        ...
        Update(prevService => ({
            ...prevService,
            [section]: prevService[section].map((item,index2) =>
                index2 === indexValue ? { ...item, [sec_value]: data?.image_full_path, [sec_image]: data?.image_path } : item
          )
        }))
        changeImage(false)
                ...
                <div>
                    <div className=' h-[10rem] relative' onDrop={(e) => handleDrop2(e)}>
                        ...
                        <div className="TeamBoxinput w-[3rem] h-[3rem] bg-[#C0CCE2] grid  place-items-center rounded-full absolute mx-auto right-0 left-0 top-[50%] transform translate-y-[-50%] cursor-pointer z-1">
                            <input type="file" onChange={(e) => handleFileUpload2(e)} className='absolute inset-0 opacity-0 cursor-pointer' />
...
export default OneImageUploadMultiple2