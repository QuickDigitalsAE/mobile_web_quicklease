...
const PromoCode = ({permission}) => {
    ...
    const onChange = (current, pageSize) => {
        ...
        if ((searchValue).trim()) {
            apiMethod2(`promo_codes/search_list/${mainLanguage}/6?page=${current}`, formdata);
        } else {
            apiMethodGet(`promo_codes/list/${mainLanguage}/6?page=${current}`, formdata);
        }
    };
    ...
    useEffect(() => {
        if (mainLanguage) {
            setCurrentPage(1)
            apiMethodGet(`promo_codes/list/${mainLanguage}/6?page=1`);
        }
    }, [mainLanguage]);
    ...
    const debounce = (func, delay) => {
      ...
      const executeApiCall = (e) => {
        ...
        if ((e.target.value).trim()) {
            apiMethod2(`promo_codes/search_list/${mainLanguage}/6?page=${currentPage}`, formdata);
        } else {
            apiMethodGet(`promo_codes/list/${mainLanguage}/6?page=${currentPage}`, formdata);
        }
      };
  ...
  return (
    ...
    <div className="TeamPageTop flex justify-between items-center">
                <h6 className='text-[1rem] mb-2 bookingSectionh relative px-3 font-Mluvka capitalize'>promo and updates</h6>
                <div className='flex gap-1'>
                    ...
                    {check("PromoCode", "PromoCode Add") && <Link to={"/promo/create"} className='bg-[#d9dcf8] py-3 px-6 rounded-full flex items-center gap-2 cursor-pointer'>
                        <img src={plus} alt="plus" />
                        <span className='font-MluvkaBold text-secondary capitalize'>Add promo</span>
                    </Link>}
                </div>
            </div>
            ...
            {resget.isLoading ? 
                        ...
                        Array.isArray(datas) && datas.map((item, index) => {
                            return (
                                <React.Fragment key={index}>
                                <PromoCodeCard permission={permission} data={item} alldata={datas} deleted={setDatas}  />
                                </React.Fragment>
                            )
                        })}
            </div>
