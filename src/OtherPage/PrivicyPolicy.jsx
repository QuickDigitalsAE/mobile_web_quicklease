...
const PrivicyPolicy = ({permission}) => {
        const [imageLoader, setImageLoader] = useState(false)
    const [resget, apiMethodGet] = useGet()
    const { mainLanguage } = useContext(MainLanguageContext);
    useEffect(() => {
        apiMethodGet(`webContents/metadata/privacy-policy/${mainLanguage}`)
    }, [mainLanguage]);
    ...
    const handleSubmit = async (values) => {
        if (imageLoader) {
            ...
            apiMethod(`webContents/metadata/privacy-policy/${mainLanguage}`, formdata)
        }
