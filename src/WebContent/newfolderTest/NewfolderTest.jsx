...
import SkeletonNewfolderTest from './SkeletonNewfolderTest'
import FormControl from '../../components/form/FormControl'

const NewfolderTest = ({permission}) => {
     const [imageLoader, setImageLoader] = useState(false)
    const [resget, apiMethodGet] = useGet()
    const { mainLanguage } = useContext(MainLanguageContext);
    useEffect(() => {
        apiMethodGet(`webContents/metadata/newfolder-test-1/${mainLanguage}`)
    }, [mainLanguage]);
    ...
    const handleSubmit = async (values) => {
        if (imageLoader) {
            ...
            apiMethod(`webContents/metadata/newfolder-test-1/${mainLanguage}`, formdata)
        }
    ...
    if (resget.isLoading || !datas) return <SkeletonNewfolderTest />
    ...
    return (
        <div className='TermsAndConditions'>
            <h2 className='text-[1.4rem] font-MluvkaBold'>Newfolder test 1</h2>
