...
import SkeletonFranchise from './SkeletonFranchise'

const WebFranchise = ({permission}) => {
     const [imageLoader, setImageLoader] = useState(false)
    const [resget, apiMethodGet] = useGet()
    const { mainLanguage } = useContext(MainLanguageContext);
    useEffect(() => {
        apiMethodGet(`webContents/metadata/franchise/${mainLanguage}`)
    }, [mainLanguage]);
    ...
    const handleSubmit = async (values) => {
        if (imageLoader) {
            ...
            formdata.append(`translation[description_two]`, datas?.description_two ?? "");
            formdata.append(`banner`, datas?.banner_value ?? "");
            apiMethod(`webContents/metadata/franchise/${mainLanguage}`, formdata)
        }
    ...
    if (resget.isLoading || !datas) return <SkeletonFranchise />
    const {description,description_two} = datas
    const initialValues = {
        ...
        <div className='TermsAndConditions'>
                ...
                <div className='bg-[#EFF4FD] p-6 rounded-3xl mb-8'>
                        ...
                        <div className='bg-[#EFF4FD] p-6 rounded-3xl mb-8'>
                            ...
                            <br />
                            <label htmlFor="description" className="mb-1 block text-[#7D8CA7] text-[.8rem]">Main Description</label>
                            <CKEditors label={"description"} data={description_two} update={(e) => handleCkChange(e,"description_two")} />
