...
import SkeletonCancelReplace from './SkeletonCancelReplace'

const CancellationReplacementPolicy = ({permission}) => {
     const [imageLoader, setImageLoader] = useState(false)
    const [resget, apiMethodGet] = useGet()
    const { mainLanguage } = useContext(MainLanguageContext);
    useEffect(() => {
        apiMethodGet(`webContents/metadata/cancellation-replacement-policy/${mainLanguage}`)
    }, [mainLanguage]);
    ...
    const handleSubmit = async (values) => {
        if (imageLoader) {
            ...
            apiMethod(`webContents/metadata/cancellation-replacement-policy/${mainLanguage}`, formdata)
        }
    ...
    if (resget.isLoading || !datas) return <SkeletonCancelReplace />
...
export default CancellationReplacementPolicy