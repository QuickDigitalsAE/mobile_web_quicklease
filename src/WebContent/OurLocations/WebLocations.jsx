...
const WebLocations = ({permission}) => {
     const [imageLoader, setImageLoader] = useState(false)
    const [resget, apiMethodGet] = useGet()
    const { mainLanguage } = useContext(MainLanguageContext);
    useEffect(() => {
        apiMethodGet(`webContents/metadata/our-locations/${mainLanguage}`)
    }, [mainLanguage]);
    ...
    const handleSubmit = async (values) => {
        if (imageLoader) {
            ...
            apiMethod(`webContents/metadata/our-locations/${mainLanguage}`, formdata)
        }
    ...
    if (resget.isLoading || !datas) return <SkeletonLocation />
