...
const EditBlogs = ({permission}) => {
  ...
  const [datas, setDatas] = useState()
    ...
    useEffect(() => {
      if(resget.data) {
        ...
        const cleanedParagraph = replaceSpecialCharacters(aaa.blog_paragraph);
    
        // Update state with cleaned data
        setDatas({
          ...aaa,
          blog_paragraph: cleanedParagraph,
        });
      }
    }, [resget.data])
  ...
  const handleSubmit = async (values) => {
   ...
   const updateData =  {
          ...
          "blog_image": datas?.blog_value ?? "",
          "blog_status": values["blog_status"].length > 0 ? 1 : 0,
      ...
      apiMethod(`blogs/update/${datas?.id}/${mainLanguage}`, updateData)
    }
  }
  };
  ...
  if (resget.isLoading || !datas) return <SkeletonCreateEdit heading={"Edit Blogs"} />;
  const {  blog_paragraph } = datas;
  let initialValues = {
    meta_title: resget?.data?.data.meta_title,
    meta_description: resget?.data?.data.meta_description,
    blog_slug: resget?.data?.data.blog_slug,
    blog_title: resget?.data?.data.blog_title,
    blog_image_alt: resget?.data?.data.blog_image_alt ?? "",
    blog_status: resget?.data?.data.blog_status && [`${String(resget?.data?.data.blog_status)}`],
    table_of_content: resget?.data?.data.table_of_content && [`${String(resget?.data?.data.table_of_content)}`],
    blog_schedule: resget?.data?.data.blog_schedule.replace(" ","T"),
  };
...
<Formik initialValues={initialValues} onSubmit={handleSubmit}>
        ...
        <div className='bg-[#EFF4FD] p-6 rounded-3xl mb-3 max-lg:p-3'>
            <div className="grid grid-cols-2 gap-2">
              ...
              {
                mainLanguage === "en" ?
                <FormControl disable name="blog_slug" label={"Slug"} placeholder="Enter Slug" className="outline-none  w-full h-[3rem] px-5 rounded-xl border border-[red]" control="input2" />
                :
                <div className='inputBox w-full mt-3'>
                <label htmlFor="Schedule" className="mb-1 block text-[#7D8CA7] text-[.8rem]">Slug</label>
                <p  className="outline-none bg-[#e7e7e7] w-full h-[3rem] px-5 rounded-xl py-3">{datas?.blog_slug}</p>
                </div>
        ...
        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
           ...
           <div>
            ...
            <FormControl name="blog_image_alt" label={"Heading Image Alt"} placeholder="Enter Blog Image Alt" className="outline-none w-full h-[3rem] px-5 rounded-xl" control="input2" />
              ...
              {
                mainLanguage === "en" ?
                <FormControl name="blog_schedule" label={"Schedule"} type={'datetime-local'} placeholder="Enter Schedule" className="outline-none w-full h-[3rem] px-5 rounded-xl" control="input2" />
                :
                <div className='inputBox w-full mt-3'>
                <label htmlFor="Schedule" className="mb-1 block text-[#7D8CA7] text-[.8rem]">Schedule</label>
                <p  className="outline-none bg-[#e7e7e7] w-full h-[3rem] px-5 rounded-xl py-3">{datas?.blog_schedule.replace(" ","T")}</p>
                </div>
